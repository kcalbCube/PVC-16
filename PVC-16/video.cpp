...
void VideoController::process(void)
{
	...
	case VIDEOCONTROLLER_MODE_1:
	{
		...
		SDL_Point points[150 * 104];
		size_t i = 0;
		auto t1 = busRead16(VIDEOCONTROLLER_T1);
		for (size_t y = 0; y < 150; ++y)
			for (size_t x = 0; x < 104 / 8; ++x)
			{
				auto byte = mc.read8(t1 + y * (104/8) + x);
				for (int j = 0; j < 8; ++j)
					if (byte & (1 << (7-j)))
...
void VideoController::setVideoMode(VideoMode vmode)
{
	...
	case VIDEOCONTROLLER_MODE_1:
	{
		window = SDL_CreateWindow("PVC-16", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 600, 416, SDL_WINDOW_SHOWN);
